...
"""
操作步骤：
* 废弃：需要先打开三星手机上的insecureADB
* 新增：需要安装busybox（root后），使用tar命令进行备份和恢复
* 修改设备串号和adb端口：f67efca1，5037
* 废弃：指定安卓用户：u0_a121
...
def backup(account):
    log('backup '+account)
    os.system('adb shell am force-stop com.tencent.mm')
    os.system('adb shell \'su -c "cd /data/data && tar cf '+account+'.tar com.tencent.mm"\'')
    os.system('adb shell "cd /mnt/sdcard && tar cf '+account+'.tar tencent"')

def restore(account):
    log('restore '+account)
    os.system('adb shell pm clear com.tencent.mm')
    log('adb shell \'su -c "cd /data/data && tar xf '+account+'.tar"\'')
    os.system('adb shell \'su -c "cd /data/data && tar xf '+account+'.tar"\'')
    os.system('adb shell "cd /mnt/sdcard && tar xf '+account+'.tar"')
